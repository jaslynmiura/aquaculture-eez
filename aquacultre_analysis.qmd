---
title: "Marine Aquaculture in Exclusive Economic Zones"
author: "Jaslyn Miura"
format: html
...
### Processing Data
```{r, output = FALSE}
# Load in Sea Surface Temperature raster.
...
```{r}
# Resampling the depth raster to match the resolution of the SST raster using the nearest neighbor approach
depth_resample <- resample(depth_crop, y = avg_sst_rast, method = "near")
```

```{r}
# Using a unit test, after resampling we expect depth_resample to have a resolution = 1.
expect_equal(resolution(depth_resample), 1)
```
...
suitable_eez_fun <- function(min_temp, max_temp, min_depth, max_depth, species_name) {
  ...
  # Calculate the area of suitable cells within west coast EEZ.
  region_area_species <- terra::extract(suitable_area_rast, west_coast_vect, fun = sum, na.rm = TRUE) %>%
  ...
  # Create a map of the suitable area within each Exclusive Economic Zone.
   species_map <- tm_graticules() +
     ...
     tm_polygons(fill = "suitable_area_bin_select_species",
                 ...
                 fill.legend = tm_legend(expression("Area km"^{2}))) +
     tm_title(text = paste("Suitable EEZ for", str_to_sentence(species_name))) +
